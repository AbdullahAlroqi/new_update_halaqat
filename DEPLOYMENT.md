...
## 🔄 تحديث النظام (للمستخدمين الحاليين)

إذا كان النظام مرفوعاً بالفعل على PythonAnywhere وتريد تحديثه بآخر التغييرات:

### 1. سحب التحديثات
افتح **Console** (Bash) في PythonAnywhere ونفذ الأوامر التالية:

```bash
# 1. الذهاب لمجلد المشروع
cd ~/new_update_halaqat

# 2. سحب آخر التحديثات من GitHub
git pull origin main
```

### 2. تحديث المكتبات وقاعدة البيانات
```bash
# 3. تثبيت المكتبة الجديدة للإشعارات
pip3 install --user pywebpush

# 4. تحديث قاعدة البيانات (إضافة جداول الإشعارات والختمات)
python3 migrate_push_update.py
python3 migrate_khatma.py
```

### 3. إعادة تشغيل التطبيق
1. اذهب إلى تبويب **Web**
2. اضغط على الزر الأخضر **Reload**
